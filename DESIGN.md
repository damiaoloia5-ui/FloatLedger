# FloatLedger — AI API 余额桌面悬浮监控 设计方案
...
在 Windows 11 桌面上显示一个常驻的半透明弹窗，实时展示 AI API（DeepSeek 等）的后台余额和当日总消费额度，风格类似 macOS 小组件（毛玻璃、圆角、小巧）。
...
```
（实际路径：`C:\Users\<用户名>\AppData\Roaming\FloatLedger\snapshot.json`）
